...
function PrizesTab() {
    ...
    const handleEdit = (prize: Prize) => {
        ...
        setValue('type', prize.type);
        setValue('value', prize.value);
        setValue('probability', prize.probability);
        setValue('quantity', prize.quantity);
        setValue('image_url', prize.image_url);
        setValue('description', prize.description);
        setValue('is_active', prize.is_active);
        setIsFormOpen(true);
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center bg-slate-900/50 p-6 rounded-3xl border border-white/5">
                <div>
                    <h1 className="text-3xl font-black tracking-tight text-white mb-2">Управление Призами</h1>
                    <p className="text-slate-400">Добавляйте и редактируйте призы для колеса фортуны</p>
                </div>
                <button
                    onClick={handleAdd}
                    className="flex items-center gap-2 bg-indigo-500 hover:bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-lg active:scale-95"
                >
            ...
            {isFormOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                    <div className="bg-[#1c1c1e] w-full max-w-lg rounded-3xl p-6 border border-white/10 text-white shadow-2xl overflow-y-auto max-h-[90vh]">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold">{editingPrize ? 'Редактировать' : 'Новый приз'}</h2>
                        ...
                        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
                            <div>
                                ...
                                <input {...register('name', { required: true })} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3" placeholder="iPhone 15" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-slate-400 mb-1">Тип</label>
                                    <select {...register('type')} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-400 mb-1">Значение/Код</label>
                                    <input {...register('value', { required: true })} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3" placeholder="50 or PROMO" />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-slate-400 mb-1">Вероятность (%)</label>
                                    <input type="number" step="0.01" {...register('probability')} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3" placeholder="10.5" />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-400 mb-1">Количество</label>
                                    <input type="number" {...register('quantity')} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3" placeholder="100" />
                                </div>
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-400 mb-1">Image URL</label>
                                <input {...register('image_url')} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3" placeholder="https://..." />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-400 mb-1">Описание</label>
                                <textarea {...register('description')} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 h-20" placeholder="Description..." />
                            </div>
                            <div className="flex items-center gap-2">
                                <input type="checkbox" {...register('is_active')} className="w-5 h-5 rounded border-white/10 bg-black/40" />
                                <label className="text-sm">Активен</label>
                            </div>
                            <button type="submit" className="w-full bg-indigo-500 py-4 rounded-xl font-bold text-white hover:bg-indigo-600">
                                Сохранить
                            </button>
                        </form>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {prizes.map((prize) => (
                    <div key={prize.id} className="bg-slate-900/50 p-5 rounded-2xl border border-white/5 relative group cursor-pointer hover:border-indigo-500/30 transition-all">
                        <div className="flex gap-4">
                            <div className="w-16 h-16 bg-white/5 rounded-xl flex items-center justify-center text-2xl overflow-hidden">
                                {prize.image_url ? (
                            ...
                            <div className="flex-1">
                                <h3 className="font-bold text-white mb-0.5">{prize.name}</h3>
                                <div className="text-xs text-slate-400">
                                    <span className="bg-white/10 px-1.5 py-0.5 rounded mr-2">{prize.type}</span>
                                    <span>{prize.value}</span>
                                </div>
                                <div className="flex items-center gap-3 mt-2 text-xs font-mono text-slate-500">
                                    <span className="text-indigo-400">{prize.probability}%</span>
                                    <span>Qty: {prize.quantity}</span>
                                    <span className={prize.is_active ? 'text-green-500' : 'text-red-500'}>
                        ...
                        <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => handleEdit(prize)} className="p-2 bg-white/10 rounded-lg hover:bg-white/20 text-white"><Edit3 className="w-4 h-4" /></button>
                            <button onClick={() => handleDelete(prize.id)} className="p-2 bg-red-500/20 rounded-lg hover:bg-red-500/40 text-red-400"><Trash2 className="w-4 h-4" /></button>
                        </div>
                    </div>
                ))}
            </div>
